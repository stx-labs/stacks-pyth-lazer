...
(define-read-only (read-price-with-staleness-check (feed-id uint))
  (let (
      (entry (unwrap! (map-get? prices feed-id) ERR_PRICE_FEED_NOT_FOUND))
...
(define-public (write (batch (list 16
  ...
  (begin
    (asserts! (is-eq contract-caller (var-get authorized-writer))
      ERR_UNAUTHORIZED
    )
...
(define-private (is-update-newer
    (feed-id uint)
    (publish-time uint)
  )
...
(define-public (set-authorized-writer (new-writer principal))
  (begin
    ...
    (print {
      type: "authorized-writer",
      action: "updated",
      data: { new-writer: new-writer },
    })
    (ok true)
  )
)
